...
import { ApiBearerAuth, ApiBody, ApiBodyOptions, ApiOperation, ApiParam, ApiTags } from '@nestjs/swagger';
...
export class UserController {
    ...
    async register(
        ...
        const { password: hashPwd, salt } = this.authService.generatePassword(password);  // 加密密码
    ...
    async login(
        ...
        // 验证密码
        const validateRes: boolean = await this.authService.validatePassWord(userPassword, userSalt, password);
